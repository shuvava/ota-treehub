...
func (store *ObjectLocalFsStore) StoreStream(ctx context.Context, ns cmndata.Namespace, id data.ObjectID, reader io.Reader) (int64, error) {
...
func (store *ObjectLocalFsStore) ReadFull(ctx context.Context, ns cmndata.Namespace, id data.ObjectID, writer io.Writer) error {
	log := store.log.WithContext(ctx)
	defer log.TrackFuncTime(time.Now())
	log.WithField("ObjectID", id).
		WithField("Namespace", ns).
...
func (store *ObjectLocalFsStore) Exists(ctx context.Context, ns cmndata.Namespace, id data.ObjectID) (bool, error) {
	log := store.log.WithContext(ctx)
	defer log.TrackFuncTime(time.Now())
	log.WithField("ObjectID", id).
		WithField("Namespace", ns).
...
func (store *ObjectLocalFsStore) namespacePath(ns cmndata.Namespace) string {
	return filepath.Join(store.root, string(ns))
}

func (store *ObjectLocalFsStore) objectPath(ctx context.Context, ns cmndata.Namespace, id data.ObjectID) (string, error) {
	log := store.log.WithContext(ctx)
