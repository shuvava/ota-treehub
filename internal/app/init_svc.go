...
func (s *Server) initDbService() {
	if s.svc.Db != nil {
		if err := s.svc.Db.Disconnect(context.Background()); err != nil {
			s.log.WithError(err).Fatal("Error on Db service distracting")
		}
	}
	...
	case db.MongoDb:
		...
		if err != nil {
			s.log.WithError(err).Fatal("Error on Db service creating")
		}
		s.svc.Db = mongoDB
		s.svc.ObjectRepo = intDb.NewObjectMongoRepository(s.log, mongoDB)
		s.svc.RefRepo = intDb.NewRefMongoRepository(s.log, mongoDB)
	default:
		s.log.WithField("type", s.config.Db.Type).Fatal("Unsupported mongoDB type")
	}
}

func (s *Server) initStorage() {
	if s.svc.ObjectStore != nil {
		s.log.Warn("Blob storage subsystem reloading")
	}
	switch blobs.Type(strings.ToLower(s.config.Storage.Type)) {
	case blobs.LocalFs:
		if s.config.Storage.Root == "" {
			s.config.Storage.Root = "/tmp"
			s.log.Warn("Blob storage root directory will be ", s.config.Storage.Root)
		}
		store, err := localfs.NewLocalFsBlobStore(s.config.Storage.Root, s.log)
		if err != nil {
			s.log.WithError(err).Fatal("Error on Storage service creating")
		}
		s.svc.ObjectStore = store
	default:
		s.log.WithField("type", s.config.Storage.Type).Fatal("Unsupported blob storage type")
	}
}
